...
def drawBoard(size, gameBoard):
	...
	for x in range(3):
		print("   --- --- ---")
		print(str(x) + " | " + gameBoard[x][0] + " | " + gameBoard[x][1] + " | " + gameBoard[x][2] + " |")
	print("   --- --- ---")
	return 
...
def checkDiagonalsForWinner(grid):
	if grid[1][1] != "-":
		...
		if diagonalOne[0] == diagonalOne[1] == diagonalOne[2]:
			return ("The winner is player " + diagonalOne[0])
		diagonalTwo = [grid[2][0], grid[1][1], grid[0][2]]
		if diagonalTwo[0] == diagonalTwo[1] == diagonalTwo[2]:
			return ("The winner is player " + diagonalTwo[0])
	else:
		return False
	...
	for x in range(3):
		if grid[x][0] != "-":
			...
			if row[0] == row[1] == row[2]:
				return ("The winner is player " + row[0])
		if grid[0][x] != "-":
			column = [grid[0][x], grid[1][x], grid[2][x]]
			if column[0] == column[1] == column[2]:
				return ("The winner is player " + column[0])
	else:
		return False	
